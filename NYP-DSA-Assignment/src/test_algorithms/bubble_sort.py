def bubble_sort(arr:list[int], reverse:bool=False) -> None:
    """
    Do a bubble sort (optimised ver) on the database by integer
    ...
    for i in range(len(arr) - 1): # -1 to stop at last element since the last element will be the highest element after an iteration from the nested for loop
        ...
        for j in range(len(arr) - i - 1): # -i to stop at last i element since they are already sorted and -1 to account for the indexing starting from 0
            if (reverse):
                # swap the elements if the jth element is smaller than the next element
                if (arr[j] < arr[j + 1]):
                    arr[j], arr[j + 1] = arr[j + 1], arr[j]
                    swapFlag = 1
            else:
                # swap the elements if the jth element is greater than the next element
                if (arr[j] > arr[j + 1]):
                    arr[j], arr[j + 1] = arr[j + 1], arr[j]
                    swapFlag = 1
